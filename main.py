...
def editProductDetails(category, productId):
    ...
    if request.method == 'POST':
        ...
        if 'provider' in login_session and login_session['provider'] \
            ...
            # check if product owner is same as logged in user or admin or not

            if product.user_id == user_id or user_id == admin_id:
                if productName and description \
                        and productCategory:
        ...
        if 'provider' in login_session and login_session['provider'] \
            ...
            user_id = check_user().id
            admin_id = check_admin().id
            if user_id == produvt.user_id or user_id == admin_id:
